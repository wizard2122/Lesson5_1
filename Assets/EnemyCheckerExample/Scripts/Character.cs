...
public abstract class Character : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        IEnumerable<Character> charaters = _characterFinder.Find();

        if (charaters != null)
            Affect(charaters.Where(character => _enemyChecker(character)));
    }

    protected abstract void Affect(IEnumerable<Character> charaters);
}